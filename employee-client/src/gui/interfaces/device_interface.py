...
class DeviceInterface(VerticalScrollInterface):
    ...
    def _create_settings_section(self) -> QWidget:
        ...
        # 标题
        title_label = BodyLabel("设备设置")
        title_label.setStyleSheet("font-weight: bold; font-size: 14px;")
        layout.addWidget(title_label)

        # 自动重连选项
        auto_reconnect_label = BodyLabel("✅ 自动重连")
        auto_reconnect_label.setStyleSheet("color: green;")
        layout.addWidget(auto_reconnect_label)

        # 操作间隔设置
        interval_label = BodyLabel("⏱️ 操作间隔: 3秒")
