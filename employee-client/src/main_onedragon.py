...
class FlowFarmApp:
    ...
    def _create_interfaces(self):
        """创建所有界面"""
        # 主页界面
        home_interface = HomeInterface(parent=self.window)
        self.window.add_sub_interface(
            home_interface, position=NavigationItemPosition.TOP
        )

        # 设备管理界面
        device_interface = DeviceInterface(parent=self.window)
        self.window.add_sub_interface(
            device_interface, position=NavigationItemPosition.TOP
        )

        # 任务管理界面
        task_interface = TaskInterface(parent=self.window)
        self.window.add_sub_interface(
            task_interface, position=NavigationItemPosition.TOP
        )
    ...
    def _connect_signals(self):
        ...
        # 获取界面实例
        home = self.window.get_interface("home_interface")
        device = self.window.get_interface("device_interface")
        task = self.window.get_interface("task_interface")

        if home and device:
            # 主页的设备管理按钮连接到设备界面
            home.device_manage_clicked.connect(
                lambda: self.window.switch_to_interface("device_interface")
            )

        if home and task:
            # 主页的开始工作按钮连接到任务界面
            home.start_work_clicked.connect(
                lambda: self.window.switch_to_interface("task_interface")
            )
